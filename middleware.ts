...
export async function middleware(request: NextRequest) {
  ...
  if (!supabaseUrl || !supabaseAnonKey) {
    // If Supabase is not configured, allow all requests but log warning
    console.warn("Supabase environment variables are not configured. Skipping authentication checks.");
    return response;
  }
  ...
  try {
    ...
    // For dashboard routes, redirect to login if middleware fails
